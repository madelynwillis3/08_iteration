...
nematode_w <- nematode %>%
  ...
  mutate(rndvi = ndvi(nir, r),
         gndvi = ndvi(nir, g),
         bndvi = ndvi(nir, b),
         rendvi= ndvi(nir, re) #normalized dist red edge
         ) %>%
...
```
```{r}
#this is used in next assignment
meancv2 <- function(df, col) { #now expects a dataframe and column. meancv just wanted a vector
  # Calculate intermediary steps of mean and sd
mean <- mean({{df}}%>% pull({{col}}), na.rm = T) #wrap arguments with curly brackets
...
meancv2(df = nematode_w, col = rndvi) #now you can choose
...
statsum <- meancv(nematode_w[[i]]) #double bracket for vector, single bracket for df.. we want df
